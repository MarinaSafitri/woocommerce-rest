	...
	function woo_custom_styling() {
		...
		if ( is_array($settings) ) {
			...
			if ( ( $settings['body_img'] != '' ) || ( $settings['body_color'] != '' ) ) {
				$output .= '.widescreen #content:before { background: transparent; } .widescreen.page-template-template-contact-php #content {
    margin-top: 53px;}' . "\n";
			}
...
if ( ( isset( $woo_options['woo_tt_live_cust'] ) && $woo_options['woo_tt_live_cust'] == 'true') ) {
	...
	add_action( 'customize_register', 'templatation_customize_register' );
	add_action( 'wp_head', 'tt_customizer_css' );
}
